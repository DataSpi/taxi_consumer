...
Weeks 1-3 all done and verified end-to-end through Airflow itself (not just manual runs). `taxi_pipeline` DAG run for Jan 2024: all 17 tasks (`extract_month`, `extract_zone_lookup`, `spark_clean_month`, `load_zone_lookup_to_snowflake`, `load_to_snowflake`, 12 Cosmos `dbt_build.*` run/test tasks) succeeded. Final state: `ANALYTICS.FCT_TRIPS` 2,723,733 rows (all correctly within Jan 2024 after the pickup-month filter), `DIM_ZONE` 265, `DIM_DATE` 366, `FCT_TRIPS_DAILY_SUMMARY` 6,662. Data scope confirmed as full calendar year **2024**. 14 real bugs were hit and fixed getting here -- full list with root causes in `docs/errors_and_fixes.md`, do not re-derive these from scratch if they resurface (e.g. the Snowflake Parquet `USE_LOGICAL_TYPE` gotcha, the Cosmos shared-virtualenv race, the flat-vs-nested `relationships` test syntax affecting Cosmos's dependency detection). Full architecture writeup: `docs/architecture.md`. Now starting Week 4 (Streamlit dashboard, polished README, GitHub Actions CI, `dbt docs`).
